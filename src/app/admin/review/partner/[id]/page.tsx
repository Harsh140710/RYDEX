  ...
  const handleApprove = async () => {
    ...
    try {
      const { data } = await axios.get(
        `/api/admin/review/partner/${id}/approve`,
      );
      console.log(data);
  ...
  const handleReject = async () => {
    ...
    try {
      const { data } = await axios.post(
        `/api/admin/review/partner/${id}/reject`,
        {
          rejectionReason,
        },
      );
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-100 to-gray-200">
      ...
      <div className="sticky top-0 z-40 backdrop-blur-xl bg-white/70 border-b">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
          <button
            onClick={() => router.back()}
            className="w-10 h-10 border flex items-center justify-center rounded-full hover:bg-gray-100 transition cursor-pointer"
          >
            <ArrowLeft size={18} />
          </button>
          ...
          <div>
            {data?.partnerStatus === "approved" ? (
              <div className="px-4 py-2 rounded-full text-xs font-semibold inline-flex items-center gap-2 bg-green-200 text-green-700">
                <CheckCircle size={14} />
                Approved
              </div>
            ) : data?.partnerStatus === "rejected" ? (
              <div className="px-4 py-2 rounded-full text-xs font-semibold inline-flex items-center gap-2 bg-red-200 text-red-700">
                <XCircle size={14} />
                Rejected
              </div>
            ) : (
              <div className="px-4 py-2 rounded-full text-xs font-semibold inline-flex items-center gap-2 bg-yellow-200 text-yellow-700">
                <Clock size={14} />
                Pending
              </div>
            )}
      ...
      {/* MAIN CONTENT */}
      <main className="max-w-7xl mx-auto px-4 py-8 grid lg:grid-cols-3 gap-10">
        {/* LEFT CONTENT */}
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">
                {vehicleDetails?.type
                  ...
                  .toUpperCase()
                  .trim()
                  .concat(vehicleDetails?.type.slice(1)) || "-"}
              </span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">
                {vehicleDetails?.vehicleModel
                  ...
                  .toUpperCase()
                  .trim()
                  .concat(vehicleDetails?.vehicleModel.slice(1)) || "-"}
              </span>
            </div>
          </AnimatedCard>
          ...
          {data?.partnerStatus === "pending" && (
            ...
            >
              ...
              <p className="text-sm text-gray-500">
                Vefiy documents carefully before approving.
              </p>
