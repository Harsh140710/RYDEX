...
function PartnerDashboard() {
  ...
  // Find current step for partner verification
  const { userData } = useSelector((state: RootState) => state.user);
  useEffect(() => {
    if (userData) setActiveStep(userData.partnerOnBoardingSteps + 1);
  }, [userData]);
      ...
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="bg-white rounded-3xl p-10 shadow-xl border overflow-x-auto">
          <div className="relative min-w-[800px]">
            <div className="absolute top-7 w-full h-[3px] bg-gray-200 rounded-full" />
            {/* Progress Bar according step complition */}
            <motion.div
              ...
              {STEPS.map((s, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flec flex-col items-center z-10 cursor-pointer"
                    onClick={() => goToStep(s)}
                  >
                    <div
        ...
        {activeStep === 5 &&
          (userData?.videoKycStatus === "approved" ? (
            ...
            <StatusCard
              ...
              description="Admin will initiate Video KYC shortly."
            />
          ))}
      </div>
    </div>
